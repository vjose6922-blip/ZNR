      ...
      if (container) {
        container.innerHTML = `
          <div style="text-align:center;padding:40px;grid-column:1/-1;">
            ...
            <button onclick="loadProducts()" style="padding:10px 24px;border-radius:40px;border:none;background:#ff4f81;color:white;font-weight:600;cursor:pointer;font-size:14px;">🔄 Reintentar</button>
          </div>`;
      }
...
function createMiniProductCard(product) {
  ...
  const stockLabel = !hasStock
    ...
    : stockNum <= 3
      ? `<span style="color:#f97316;font-size:11px;">⚠️ Últimas ${stockNum}</span>`
      : `<span style="color:#22c55e;font-size:11px;">✓ En stock</span>`;
  ...
  card.innerHTML = `
    ...
    <div class="product-info" style="padding:12px;">
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-top:6px;flex-wrap:wrap;">
        ...
        <span title="Envío disponible" style="font-size:11px;color:#3b82f6;">🚚</span>
      </div>
      <div class="product-actions" style="margin-top:8px;">
        <button class="primary-button mini-add-btn" style="padding:8px 12px;font-size:12px;"${!hasStock ? ' disabled style="opacity:0.5;cursor:not-allowed;"' : ''}>🛒 Añadir</button>
      </div>
    </div>
  `;
...
function renderRecentProducts() {
  ...
  // Inject "Limpiar recientes" button in the section header if not already present
  ...
  container.innerHTML = recentProducts.map(product => {
    ...
    const hasStock = stockNum > 0;
    const stockLabel = !hasStock
      ? `<span style="color:#ef4444;font-size:11px;">Sin stock</span>`
      : stockNum <= 3
        ? `<span style="color:#f97316;font-size:11px;">⚠️ Últimas ${stockNum}</span>`
        : `<span style="color:#22c55e;font-size:11px;">✓ En stock</span>`;
    const sizesHtml = product.Talla
    ...
    const genderRaw = (GENDER_BY_CATEGORY[product.Categoria] || '').toLowerCase();
    const genderLabel = genderRaw === 'hombre' ? '👔 Caballero' : genderRaw === 'mujer' ? '👗 Dama' : '';
    const shippingIcon = `<span title="Envío disponible" style="font-size:11px;color:#3b82f6;">🚚</span>`;
    const descHtml = product.Descripcion
...
window.homeLooks = homeLooks;   // actualiza con los nuevos looks
}
...
function setupEventListeners() {
  ...
  window.addEventListener('theme-toggle', () => {
    // El botón puede haberse renombrado a up-open-btn por user-preferences.js
    // En ese caso no hay que cambiar su texto (ya es ⚙️)
