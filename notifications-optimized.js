...
function processAndRenderNotifications(data) {
  ...
  // Agregar UI de filtros si no existe
...
function createOptimizedNotificationCard(group) {
  ...
  // Verificar si hay productos pendientes (con status pending)
  const hasPendingItems = group.notifications.some(n => n.status === 'pending');
